...
function smtp_mail( $sendto_email, $subject, $body, $extra_hdrs, $user_name){  
	...
	$mail->Username = "web@moviebird.cn"; // SMTP username  注意：普通邮件认证不需要加 @域名    
    $mail->Password = "Wehateboss999"; // 我的邮箱密码（发件人）  
    $mail->From = "web@moviebird.cn";      // 发件人邮箱  
    $mail->FromName =  "web@moviebird.cn";  // 发件人  
  
    $mail->CharSet = "UTF-8";   // 这里指定字符集！  
    $mail->Encoding = "base64";  
    $mail->AddAddress($sendto_email,$sendto_email);  // 收件人邮箱和姓名  
    $mail->AddReplyTo("web@moviebird.cn","web@moviebird.cn");  
...
//根据经验值计算等级
function get_grade($expval){
	$expval = intval($expval);
	
	$grade = 1;
	if($expval < 10){
		$grade = 1;
	}else if($expval >= 10 && $expval < 20){
		$grade = 2;
	}else if($expval >= 30 && $expval < 80){
		$grade = 3;
	}else if($expval >= 80 && $expval < 200){
		$grade = 4;
	}else if($expval >= 200 && $expval < 600){
		$grade = 5;
	}else if($expval >= 600){
		$grade = 6;
	}
...
function get_category_id($category){
	...
	$row = $dosql->GetOne("SELECT id FROM `#@__category` WHERE title = '".$category."'");
	if(!empty($row['id'])){
		return $row['id'];	
	}else{
		return '';	
...
function get_attr_id($attr){
	...
	$row = $dosql->GetOne("SELECT id FROM `#@__attr` WHERE title = '".$attr."'");
	if(!empty($row['id'])){
		return $row['id'];	
	}else{
		return '';	
...
function get_username($uid){
	...
	$row = $dosql->GetOne("SELECT username FROM `#@__member` WHERE id = '".$uid."'");
...
function get_tag($tag_id){
	...
	$row = $dosql->GetOne("SELECT title FROM `#@__tag` WHERE id = ".$tag_id);
...
//获取用户报名活动的个数
function get_active_number($uid){
	global $dosql;
	
	$row = $dosql->GetOne("SELECT count(*) as num FROM `#@__active_sign` WHERE uid = ".$uid);
