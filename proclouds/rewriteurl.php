...
RewriteCond %{QUERY_STRING} ^(.*)$
RewriteRule ^(.*)/contact-([0-9]+)\.html$ $1/contact.php?page=$2
RewriteCond %{QUERY_STRING} ^(.*)$
...
RewriteCond %{QUERY_STRING} ^(.*)$
RewriteRule ^contact-([0-9]+)\.html$ contact.php?page=$1
RewriteCond %{QUERY_STRING} ^(.*)$
...
&lt;/rule&gt;
&lt;rule name="message"&gt;
	&lt;match url="^(.*/)*contact-([0-9]+)\.html$" /&gt;
	&lt;action type="Rewrite" url="{R:1}/contact.php\?page={R:2}" /&gt;
&lt;/rule&gt;
&lt;rule name="contact"&gt;
...
rewrite ^([^\.]*)/contact-([0-9]+)\.html$ $1/contact.php?page=$2 last;
...
<div class="copyright">© <?php echo date('Y');?> ProCloud All Rights Reserved.</div>
</body>
</html>