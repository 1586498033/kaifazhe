<?php	
require_once(dirname(__FILE__).'/inc/config.inc.php');IsModelPriv('infoimg');
...
if($action == 'add')
{
	...
	//保存远程缩略图
	if($rempic=='true' &&
	   preg_match("#^http:\/\/#i", $case_img))
	{
		$case_img = GetRemPic($case_img);
	}
	//保存远程资源
	if($remote == 'true')
	{
	...
	if($autothumb == 'true')
	{
		...
		{
			$case_img = $imgurl[1][0];
			$case_img = substr($case_img, strpos($case_img, 'uploads/'));
		}
	}
	//自动提取内容到摘要
	if($autodesc == 'true')
	...
	if(!empty($r['picwidth']) &&
	   !empty($r['picheight']))
	{
		ImageResize(PHPMYWIND_ROOT.'/'.$case_img, $r['picwidth'], $r['picheight']);
	}
	$description = str_replace(array("\n", "\r\n"),"<br />",$description);
	$sql = "INSERT INTO `$tbname` (siteid, classid, parentid, parentstr, mainid, mainpid, mainpstr, type, title, stitle, colorval, boldval, flag, tag, source, author, linkurl, keywords, description, content, case_img, case_bimg, case_video, picarr, orderid, hits, posttime, checkinfo {$fieldname}) VALUES ('$cfg_siteid', '$classid', '$parentid', '$parentstr', '$mainid', '$mainpid', '$mainpstr', '$type', '$title', '$stitle', '$colorval', '$boldval', '$flag', '$tag', '$source', '$author', '$linkurl', '$keywords', '$description', '$content', '$case_img', '$case_bimg', '$case_video', '$picarr', '$orderid', '$hits', '$posttime', '$checkinfo' {$fieldvalue})";
...
else if($action == 'update')
{
	...
	$description = str_replace(array("\n", "\r\n"),"<br />",$description);
	$sql = "UPDATE `$tbname` SET siteid='$cfg_siteid', classid='$classid', parentid='$parentid', parentstr='$parentstr', mainid='$mainid', mainpid='$mainpid', mainpstr='$mainpstr', type='$type', title='$title', stitle='$stitle', colorval='$colorval', boldval='$boldval', flag='$flag', tag='$tag', source='$source', author='$author', linkurl='$linkurl', keywords='$keywords', description='$description', content='$content', case_img='$case_img', case_bimg='$case_bimg', case_video='$case_video', picarr='$picarr', orderid='$orderid', hits='$hits', posttime='$posttime', checkinfo='$checkinfo' {$fieldstr} WHERE id=$id";
	if($dosql->ExecNoneQuery($sql))
	{
		header("location:$gourl");
		exit();
