<?php	
require_once(dirname(__FILE__).'/inc/config.inc.php');IsModelPriv('infoclass');
...
if($action == 'add')
{
	...
	$sql = "INSERT INTO `$tbname` (siteid, parentid, parentstr, infotype, classname, eclassname, linkurl, picurl, picwidth, picheight, seotitle, keywords, description, orderid, checkinfo) VALUES ('$cfg_siteid', '$parentid', '$parentstr', '$infotype', '$classname', '$eclassname', '$linkurl', '$picurl', '$picwidth', '$picheight', '$seotitle', '$keywords', '$description', '$orderid', '$checkinfo')";
...
else if($action == 'update')
{
	...
	if($parentid != $id)
	{
		...
		$sql = "UPDATE `$tbname` SET siteid='$cfg_siteid', parentid='$parentid', parentstr='$parentstr', infotype='$infotype', classname='$classname', eclassname='$eclassname', linkurl='$linkurl', picurl='$picurl', picwidth='$picwidth', picheight='$picheight', seotitle='$seotitle', keywords='$keywords', description='$description', orderid='$orderid', checkinfo='$checkinfo' WHERE id=$id";
		if($dosql->ExecNoneQuery($sql))
		{
			header("location:$gourl");
			exit();
