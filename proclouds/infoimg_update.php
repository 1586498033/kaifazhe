...
?>
<div class="formHeader"> <span class="title">修改案例</span> <a href="javascript:location.reload();" class="reload">刷新</a> </div>
<form name="form" id="form" method="post" action="infoimg_save.php" onsubmit="return cfm_infolm();">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formTable">
		...
		?>
		<tr>
			<td height="40" align="right">分　类：</td>
		...
		<tr>
			<td height="40" align="right">S标　题：</td>
			<td><input type="text" name="stitle" id="stitle" class="input" value="<?php echo $row['stitle']; ?>" <?php echo 'style="color:'.$row['colorval'].';font-weight:'.$row['boldval'].';"'; ?> />
				<span class="maroon">*</span>
				<div class="titlePanel">
					<input type="hidden" name="colorval" id="colorval" value="<?php echo $row['colorval']; ?>" />
					<input type="hidden" name="boldval" id="boldval" value="<?php echo $row['boldval']; ?>" />
		...
		<tr class="nb">
			...
			$dosql->Execute("SELECT * FROM `#@__type` ORDER BY orderid ASC");
			while($r = $dosql->GetArray())
			{
				echo '<span><input type="checkbox" name="type[]" id="type[]" value="'.$r['typename'].'"';
				if(in_array($r['typename'],$typearr))
				{
		...
		<tr class="nb">
			<td height="40" align="right">标　签：</td>
			<td class="attrArea"><?php
			$tagarr = explode(',',$row['tag']);
			$dosql->Execute("SELECT * FROM `#@__tag` ORDER BY orderid ASC");
			while($r = $dosql->GetArray())
			{
				echo '<span><input type="checkbox" name="tag[]" id="tag[]" value="'.$r['id'].'"';

				if(in_array($r['id'],$tagarr))
				{
					echo 'checked="checked"';
				}
		...
		<tr>
			<td height="40" align="right">小缩略图：</td>
			<td><input type="text" name="case_img" id="case_img" class="input" value="<?php echo $row['case_img']; ?>" />
				<span class="cnote"><span class="grayBtn" onclick="GetUploadify('uploadify','缩略图上传','image','image',1,<?php echo $cfg_max_file_size; ?>,'case_img')">上 传</span> <span class="rePicTxt">
				<input type="checkbox" name="rempic" id="rempic" value="true" />
				远程</span> <span class="cutPicTxt"><a href="javascript:;" onclick="GetJcrop('jcrop','picurl');return false;">裁剪</a></span> </span>[<span style="color:red;">后台显示</span>]</td>
		</tr>
		<tr>
			<td height="40" align="right">大缩略图：</td>
			<td><input type="text" name="case_bimg" id="case_bimg" class="input" value="<?php echo $row['case_bimg']; ?>" />
				<span class="cnote"><span class="grayBtn" onclick="GetUploadify('uploadify','缩略图上传','image','image',1,<?php echo $cfg_max_file_size; ?>,'case_bimg')">上 传</span> <span class="rePicTxt">
				<input type="checkbox" name="rempic" id="rempic" value="true" />
		...
		<tr>
			...
			<td><textarea name="description" id="description" class="textdesc"><?php echo preg_replace("/<(.*?)>/","",$row['description']); ?></textarea>
				<div class="hr_5"></div>
				最多能输入 <strong>255</strong> 个字符 </td>
		</tr>
		<tr>
			<td height="40" align="right">案例视频：</td>
			<td>
				<input type="text" name="case_video" id="case_video" class="input" value="<?php echo $row['case_video']; ?>" />
				<span class="cnote">
					<span class="grayBtn" onclick="GetUploadify('uploadify','视频上传','media','media',1,<?php echo $cfg_max_file_size; ?>,'case_video')">上 传</span> 
				</span>
			</td>
		</tr>
		<tr>
